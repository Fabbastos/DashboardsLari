...
st.markdown("""
    <style>
    /* Força o texto para uma cor legível nos cards */
...
# 7. Grid de Gráficos 2x2 (COM FONTES CLARAS FORÇADAS)
st.markdown("### 📈 Visualizações Gerenciais")
c1, c2 = st.columns(2)
c3, c4 = st.columns(2)

altura_graficos = 320 
cor_fonte_clara = "#FFFFFF" # Branco puro para garantir o contraste no modo escuro
...
with c1:
    ...
    fig_funil.update_traces(
        ...
        textfont=dict(color=cor_fonte_clara) # Força a fonte dos números
    )
    fig_funil.update_layout(
        height=altura_graficos, margin=dict(l=10, r=10, t=40, b=10),
        font=dict(color=cor_fonte_clara) # Força a fonte do título e eixos
    )
...
with c2:
    ...
    fig_origem.update_traces(textfont=dict(color=cor_fonte_clara))
    fig_origem.update_layout(
        height=altura_graficos, margin=dict(l=10, r=10, t=40, b=10), showlegend=False,
        font=dict(color=cor_fonte_clara)
    )
...
with c3:
    fig_cat = px.bar(df.groupby("Categoria")["Valor"].sum().reset_index().sort_values("Valor"),
                     ...
                     text_auto='.2s', color="Categoria", color_discrete_sequence=px.colors.qualitative.Bold)
    fig_cat.update_traces(textfont=dict(color=cor_fonte_clara))
...
with c4:
    fig_idade = px.bar(df.groupby("Idade")["Valor"].sum().reset_index().sort_values("Valor"),
                       ...
                       text_auto='.2s', color="Idade", color_discrete_sequence=px.colors.qualitative.Pastel)
    fig_idade.update_traces(textfont=dict(color=cor_fonte_clara))
...
def color_ok(val):
    return 'background-color: #d4edda; color: #155724;' if val == 'Ok' else ''
